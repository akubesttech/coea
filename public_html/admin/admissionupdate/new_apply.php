...
authorize($_SESSION["access3"]["adm"]["nsp"]["delete"]) ) {
 ...
 if(empty($dep1)){ $links = "new_apply.php";}else{ $links = "new_apply.php?dept1_find=".$dep1."&session2=".$sec1."&c_choice=".$los;}
 ?>
...
for($i=0; $i < $N; $i++){$resultd = mysqli_query($condb,"DELETE FROM new_apply1 where stud_id='$id[$i]'");}
	message(" Applicant(s) information  Successfully Deleted .", "success");
...
for($i=0; $i < $N; $i++){$resultapp = mysqli_query($condb,"UPDATE new_apply1 SET verify_apply ='TRUE' where stud_id = '$id[$i]'");}
...
if(isset($_GET['details'])){
 /* $vstatus = $_GET['nst'];
if($newstatus1 == 'Aprove'){ $is_active20 = $vstatus == 'Aprove' ? 'TRUE' : 'FALSE';
}else{ $is_active20 = $vstatus == 'Cancel Aproval' ? 'FALSE' : 'TRUE'; }
	//$sql1 = "SELECT * FROM new_apply1  WHERE stud_id ='$_GET[userId]' and verify_apply = '".safee($condb,$is_active20)."'";
	$sql1 = "SELECT * FROM new_apply1  WHERE stud_id ='$_GET[userId]' and verify_apply = '".safee($condb,$is_active20)."'";
$qsql1=mysqli_query($condb,$sql1);
$dform_checkexist20 = mysql_num_rows($qsql1); 
if($dform_checkexist20 < 1){ //message("The page you are trying to access is not Available.", "error");
redirect('new_apply.php'); }else{  //statusUser(); 
}
*/
 //redirect('new_apply.php?details&userId='.$_GET['userId'].'&nst='.$vstatus);
...
$num_fchoice =$find_choicead['course_choice'] ;$verif  =$find_choicead['verify_apply']; $modeentry2 = $find_choicead['moe']; $entrylev2 = getelevel($modeentry);
