...
$refme = $tranx->data->reference ;
  $qpayn = mysqli_query($condb,"SELECT * FROM payment_tb WHERE  trans_id ='".safee($condb,$refme)."'");
	$hpeno = mysqli_fetch_array($qpayn); $feetp1 = $hpeno['fee_type']; $appnon = $hpeno['app_no'];
if('success' == $tranx->data->status){
  // transaction was successful...
  // please check other things like whether you already gave value for this ref
  // if the email matches the customer who owns the product etc
  // Give value
$sql2_up = mysqli_query($condb,"UPDATE payment_tb SET pay_status='1',paid_amount ='".safee($condb,$payamount)."' WHERE trans_id ='".safee($condb,$refme)."' ")or die(mysqli_error($condb));
