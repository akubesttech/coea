...
mysqli_query($condb,"update grade_tb SET prog='".safee($condb,$f_pro)."',grade_group='".safee($condb,$grade_group)."',b_min='".safee($condb,$min_grade)."',b_max='".safee($condb,$max_grade)."',grade = '".safee($condb,$grade)."' ,gp = '".safee($condb,$gradepoint)."',gradename = '".safee($condb,$graderemark)."' where id='".safee($condb,$get_RegNo)."'") or die(mysqli_error($condb));
// ob_start();
...
<input type="text"  id="max" name="max" value="<?php echo ($row_upform['b_max']); ?>" placeholder="0" onkeypress="return isNumber(event);"   class="form-control" >
                	...
                	<div class="form-group"><select   class="form-control" name="grade" required="required" ><option selected="selected" value="<?php echo $row_upform['grade']; ?>" ><?php echo $row_upform['grade']; ?></option><option  value="A">A</option><option  value="B">B</option><option  value="C">C</option><option  value="D">D</option><option  value="E">E</option><option  value="F">F</option>
</select></div>
